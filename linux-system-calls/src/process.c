 ...
 *
 * Small example of fork, wait and exit system calls.  We
 * fork a child process, display pid of process in both
 * parent and child.  The child sleeps and returns a
 * status back to parent using the exit call.  The
 * parent wiats on the child to exit and displays status
 * received from child.
 *
...
int main(int argc, char** argv)
{
  ...
  // fork returns 2 times, once in original process, and once in
  ...
  if (pid == 0) // pid was 0 so we are the child
  {
    ...
    printf("child pid: %d\n", getpid());
    printf("child process is sleeping for a bit\n");
    sleep(10);
    printf("child process exiting\n");
    
    // return an exit status back to parent on child exit
    exit(33);
  }
